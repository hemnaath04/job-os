...
// The staleness reading, including why the ceiling is what it is, now lives
// in lib/agent-job-stale.ts so the MCP connector can share it.
...
export const appwriteWorkspace = {
  ...
  async getAgentJob<T = unknown>(jobId: string): Promise<AgentJob<T>> {
    ...
    // Shared with the MCP connector rather than inlined here. The two used to
    // disagree: this surface read a dead run correctly and the connector
    // returned the raw snapshot, so an agent polling the same job was told
    // "running" indefinitely. See lib/agent-job-stale.ts.
    return withStaleness(parseSnapshot<AgentJob<T>>(row));
  },
